...
{	
	...
	static  Logger logger =Logger.getLogger(IconReader.class);
	//private XSSFSheet Sheet;
	//private XSSFRow row;
	//private XSSFCell cell;
	//private int iIndex;
	//private static String sFileName1 = null;
	private static IconReader icnReader = null;
	...
	public static IconReader getInstance(){
		if (icnReader == null){
			icnReader = new IconReader(HTML.properties.getProperty("IconSheetName"));
		}
		return icnReader;
	}
	 ...
	 * @param sFileName
	 */
	private IconReader(String sFileName)
	{
...
//					String cellText  = String.valueOf(cell.getNumericCellValue());
	...
	public boolean addTestCasesFromDataSheetName(ConcurrentLinkedQueue<String> testCases) throws Exception
	{
	      ...
	      IconReader sXL = IconReader.getInstance(); //new XlsxReader(DataSheetName);
	...
	public synchronized boolean  executeUpdateQuery(String sheetName, HashMap<String,Object> updateColumnNameValues, HashMap<String,Object> whereConstraint ) {
		...
		boolean status = false;
		try {
			IconReader sXL = IconReader.getInstance();
