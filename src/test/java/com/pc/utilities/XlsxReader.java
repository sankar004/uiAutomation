...
//			String sFilePath = dir + "\\Data\\" + PCThreadCache.getInstance().getProperty("DataSheetName") + ".xlsm";
	...
	public void failuresReader(String sExpected,String sAcutal, String sElementName, String sElementProperty, String sException) throws Exception
	{
		String sheetName = "FailureHub";
		XlsxReader sXL = XlsxReader.getInstance();
		int rowcount = sXL.getRowCount(sheetName);
		sXL.setCellData(sheetName, PCThreadCache.getInstance().getProperty("TCID"), rowcount, 0);
		sXL.setCellData(sheetName, PCThreadCache.getInstance().getProperty("testcasename"), rowcount, 1);
		sXL.setCellData(sheetName, PCThreadCache.getInstance().getProperty("methodName"), rowcount, 2);
		sXL.setCellData(sheetName, sExpected, rowcount, 4);
		sXL.setCellData(sheetName, sAcutal, rowcount, 5);
		sXL.setCellData(sheetName, sElementName, rowcount, 6);
		sXL.setCellData(sheetName, sElementProperty, rowcount, 7);
		sXL.setCellData(sheetName, sException, rowcount, 8);
